...
We wstêpnym opisie powiedziane by³o, ¿e technologia CUDA zapocz¹tkowana by³a w uk³adach graficznych serii GeForce, Tesla oraz Quadro. Poni¿ej przedstawiona zosta³a tabela ukazuj¹ca oficjalne wsparcie okreœlonej wersji CUDA w poszczególnych uk³adach graficznych:
\ref{tab:wspierane_karty}
\newpage
\begin{table}[h]
\centering
...
Kolejn¹ wa¿n¹ rzecz¹ wyró¿niaj¹ca karty graficzne jest ich zdolnoœæ obliczeniowa (ang. compute capability). Identyfikuje ona mo¿liwoœci obliczeniowe danej karty graficznej w odniesieniu do technologii NVIDIA CUDA. Poni¿ej przedstawiona zosta³a tabela ukazuj¹ca mo¿liwoœci kart graficznych w zale¿noœci od profilu CUDA:
\ref{tab:porownanie_zdolnosci}
\newpage
\begin{table}[h]
\centering
...
Na obrazku \ref{fig:multiprocesor} poni¿ej przedstawiony zosta³ pogl¹dowy schemat pamiêci w architekturze CUDA.
\begin{figure}[h]
	\centering
...
\section{Przyk³adowy kod pod architekturê CUDA}
\label{sec:chapter4:kod}
\begin{lstlisting}[language=C,style=outcode]
#include <stdlib.h>
#include <cuda_runtime.h>
#include <cutil.h>
// definicja funkcji obliczeniowej (wykorzystanie s³owa kluczowego __global__)
__global__ void testKernel(float *data)
{
int id = blockIdx.x * blockDim.x + threadIdx.x; // oblicz indeks w tablicy
data[id] = sqrt(data[id]); // oblicz funkcjê sqrt() dla danego elementu tablicy
}
int main()
{
cudaSetDevice(0); // inicjalizacja urz¹dzenia CUDA
float *hData = (float*)malloc(sizeof(float) * ARRAY_SIZE);
float *dData;
cudaMalloc((void**)&dData, sizeof(float) * ARRAY_SIZE); // alokacja pamiêci CUDA
InitializeData(hData); // inicjalizacja tablicy hData
// skopiowanie tablicy hData do pamiêci CUDA reprezentowanej przez dData
cudaMemcpy(dData, hData, sizeof(float) * ARRAY_SIZE, cudaMemcpyHostToDevice)
dim3 dimBlock(BLOCK_SIZE, 1); // ustalenie wielkoœci bloku 1D
dim3 dimGrid(ARRAY_SIZE / dimBlock.x, 1); // ustalenie wielkoœci kraty 1D
testKernel<<<dimGrid, dimBlock>>>(dData); // wywo³anie funkcji obliczeniowej
// skopiowanie wynikowej zawartoœci tablicy dData do hData
cudaMemcpy(dData, hData, sizeof(float) * ARRAY_SIZE, cudaMemcpyDeviceToHost);
return 0;
}
\end{lstlisting}
